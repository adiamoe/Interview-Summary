...
1. 提高内存的利用率。操作系统可以将进程实际使用的虚拟地址映射到物理地址，其他数据存放在磁盘中，需要时再读取
2. 提高了进程的隔离性和安全性。每个进程仅能看到自己的虚拟地址空间，保障了隔离性。
3. 透明性：每个进程看到的地址是连续统一的，降低了编程的复杂度
...
1. 申请的虚拟地址没有被分配，不属于当前进程的虚拟内存区域(VMA)。违反了隔离性和安全性，直接杀掉该进程。
2. 虚拟页处于已分配但未映射至物理内存的状态：
    * 对应的物理页被写入磁盘，不在物理内存中，触发缺页中断后cpu运行缺页异常处理函数，将对应的数据重新加载到物理页。之后，cpu重新执行本条指令。
    * 当应用程序申请分配内存时，操作系统可选择将新分配的虚拟页标记成**已分配但未映射至物理内存**状态。当应用程序访问这个虚拟页的时候，触发缺页中断，再为该页分配物理页。当使用`fork()`函数创建子进程时，子进程的虚拟内存也指向父进程的物理页，直到有一方要修改数据再分配物理内存。这被称为**写时复制(copy on write)**

如何区别两种缺页中断：虚拟内存是否属于VMA
...
* 同步是有一次请求返回一次请求
* 异步可以向同一个对象多次传递请求，准备好之后通知调用者

阻塞和非阻塞关注的是**程序在等待调用结果时的状态**：

* 阻塞是指调用者一直等待，直到被调用的对象返回结果
* 非阻塞是是调用后直接返回，如果没有准备好则返回一个错误
...
malloc是内存中通用的分配内存的手段，需要调用标准库，进入内核态完成分配，效率较低。内存池是针对某种场景定制的，一次性获取大块内存，由应用自身进行内存的申请和释放。
...
aba问题：CAS需要在操作值的时候检查下值有没有发生变化，如果没有发生变化则更新，但是如果一个值原来是A，变成了B，又变成了A，那么使用CAS进行检查时会发现它的值没有发生变化，但是实际上却变化了。ABA问题的解决思路就是使用版本号。在变量前面追加上版本号，每次变量更新的时候把版本号加一，那么A－B－A 就会变成1A-2B-3A。
...
将LFS简化为一个日志，讨论其中的读写和恢复：

![Log](E:\Repository\knowledge\Log.png)

对于一个事务有五种行为：

1. **开始**：分配一个新的事务号
2. **写**：添加一个新的记录到日志
3. **读**：搜索整个日志寻找最近的一次提交，读取其中的值
4. **提交(commit)**：对于原子性最重要的一步，将nothing转换成all。(原子性由磁盘支撑)
5. **放弃(abort)**：中止此次事务

对于这样一个简单的日志系统，评价它的性能：

1. 写的性能还不错：是顺序写，不需要转动磁盘
2. 恢复性能非常好：不需要干任何事
3. 读性能很糟糕：每次读都需要遍历整个日志
...
1. 读性能很好，可以直接读取cell storage中的值
2. 写性能一般，每次写要写两次磁盘，更新cell和log
3. 恢复需要扫描整个log
...
1. 在日志中记录<START CKPT(T1...Tk)>，即记录还在活动的事务。
2. 等待这些事务全部提交或中止，但同时允许新事务的开始
3. 写入<END CKPT>

在恢复时如果日志中存在<END CKPT>，则只需要遍历到<START>，如果只有<START CKPT(T1...Tk)>，则需要遍历到(T1....Tk)开始的时候。
